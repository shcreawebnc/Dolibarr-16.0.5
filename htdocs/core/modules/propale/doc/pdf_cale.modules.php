 ...
 */
class pdf_cale extends ModelePDFPropales
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "cale";
		$this->description = $langs->trans('DocModelCaleDescription');
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->propal->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
				...
				// Affiche zone totaux
				$posy = $this->_tableau_tot($pdf, $object, 0, $bottomlasttab, $outputlangs, $totalQty);
	 ...
	 */
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs, $totalQty, $outputlangsbis = null)
	{
		// phpcs:enable
		global $conf, $mysoc;
		$default_font_size = pdf_getPDFFontSize($outputlangs);
		...
		$pdf->MultiCell($largcol2, $tab2_hl, preg_replace('~\,00+$~','',price($total_ht + (!empty($object->remise) ? $object->remise : 0), 0, $outputlangs)), 0, 'R', 1);
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT)) {
			...
			if (!empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_IFNULL) && $tvaisnull) {
				...
				foreach ($this->localtax1 as $localtax_type => $localtax_rate) {
					...
					foreach ($localtax_rate as $tvakey => $tvaval) {
						if ($tvakey != 0) {    // On affiche pas taux 0
							...
							$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
							$pdf->MultiCell($largcol2, $tab2_hl, preg_replace('~\,00+$~','',price($tvaval, 0, $outputlangs)), 0, 'R', 1);
						}
					}
				}
				//}
				...
				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($largcol2, $tab2_hl, preg_replace('~\,00+$~','',price($total_ttc, 0, $outputlangs)), $useborder, 'R', 1);
			}
		}
		...
		*/
		...
		if ($deja_regle > 0) {
			...
			$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($largcol2, $tab2_hl, preg_replace('~\,00+$~','',price($deja_regle, 0, $outputlangs)), 0, 'R', 0);
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '')
	{
		...
		if (empty($hidetop)) {
			...
			dol_syslog($this->colwidthref,LOG_NOTICE);
		...
		*/
		...
		if ($showaddress) {
			...
			/*if (empty($conf->global->MAIN_PDF_HIDE_SENDER_NAME)) {
				...
				$pdf->MultiCell(80, 4, '', 0, $ltrdirection);
				$posy = $pdf->getY();
			}*/
			...
			$pdf->MultiCell($widthrecbox - 2, 4, $carac_client_name, 0, $ltrdirection);
