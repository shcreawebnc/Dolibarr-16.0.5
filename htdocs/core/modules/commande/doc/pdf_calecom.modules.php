 ...
 */
class pdf_calecom extends ModelePDFCommandes
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "calecom";
		$this->description = $langs->trans('PDFEinsteinCalecomDescription');
		...
		if ($conf->commande->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					$this->pdf_writelinedesc_cale($pdf, $object, $i, $outputlangs, $this->posxtva - $curX, 3, $curX, $curY, $hideref, $hidedesc);
					...
					if ($pageposafter > $pageposbefore) {	// There is a pagebreak
						...
						$this->pdf_writelinedesc_cale($pdf, $object, $i, $outputlangs, $this->posxtva - $curX, 4, $curX, $curY, $hideref, $hidedesc);
						$pageposafter = $pdf->getPage();
						$posyafter = $pdf->GetY();
				...
				// Display total zone
				$posy = $this->_tableau_tot($pdf, $object, $deja_regle, $bottomlasttab, $outputlangs, $totalQty);
		...
		*/
		...
		if ($showaddress) {
			...
			if (empty($conf->global->MAIN_PDF_HIDE_SENDER_NAME)) {
				...
				$pdf->SetFont('', 'B', $default_font_size);
				$pdf->MultiCell(80, 4, /*$outputlangs->convToOutputCharset($this->emetteur->name)*/$sender_name, 0, $ltrdirection);
				$posy = $pdf->getY();
			}
