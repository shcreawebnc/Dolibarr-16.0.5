...
/*
VARIABLE EXTRAFIELD DOCUMNT : $object->array_options ['options_interfacepdf']
VARIABLE EXTRAFIELD SOCIETE : 

	@require_once DOL_DOCUMENT_ROOT . '/societe/class/societe.class.php';

	$extrafields = new ExtraFields($this->db);
	$extralabels = $extrafields>fetch_name_optionals_label('societe_extrafields', true);               

	if (count($extralabels) > 0) {     

			$societe = new Societe($this->db);
			$societe->fetch_optionals($soc_id);
			$societe->fetch_optionals($objet->fk_soc); Voir la table propale
			$activite = $societe->array_options['options_interfacepdf'])); 
...
/**
 *	Class to generate the customer invoice PDF with template Cale
 */
class pdf_cale extends ModelePDFFactures
{
	/**
	...
	/**
	 * @var float X position for column
	 */
	public $posxref;
	public $posxdesc;
	public $posxqty;
	...
	public $colwidthrem;
	public $colwidthprht;
	public $colwidthtotalht;
	public $colwidthtvatx;
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "cale";
		$this->description = $langs->trans('PDFCaleDescription');
		...
		$this->colwidthref = 25;
		$this->colwidthdesc = 80;
		$this->colwidthqty = 15;
		$this->colwidthpuht = 15;
		$this->colwidthrem = 15;
		$this->colwidthprht = 15;
		$this->colwidthtotalht = 15;
		$this->colwidthtvatx = 15;



		$this->posxref = $this->marge_gauche + 1;
		$this->posxdesc = $this->posxref + $this->colwidthref;
		$this->posxqty = $this->posxdesc + $this->colwidthdesc;
		$this->posxpuht = $this->posxqty + $this->colwidthqty;
		$this->posxrem = $this->posxpuht + $this->colwidthpuht;
		$this->posxrht = $this->posxrem + $this->colwidthrem;
		$this->posxtotalht = $this->posxrht + $this->colwidthprht;
		$this->posxtvatx = $this->posxtotalht + $this->colwidthtotalht;
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->facture->dir_output) {
			...
			if (file_exists($dir)) {
				...
				$heightforfooter = $this->marge_basse + 21; // Height reserved to output the footer (value include bottom margin)
				...
				$tab_top = 100 + $top_shift;		// top_shift is an addition for linked objects or addons (0 in most cases)
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					$this->pdf_writelinedesc_cale($pdf, $object, $i, $outputlangs, $this->colwidthdesc - 4, 3, $curX, $curY, $hideref, $hidedesc);
					$noQty = $this->pdf_writelinedesc_cale($pdf, $object, $i, $outputlangs, $this->colwidthdesc - 4, 3, $curX, $curY, $hideref, $hidedesc);
					//var_dump($noQty);exit;
					...
					$pdf->SetFont('', '', $default_font_size - 3); // We reposition the default font

					// Reference of product line
					$curX = $this->posxref;
					$barcode = $this->pdf_getlinebarcode($object, $i, $outputlangs, $hidedetails);
					$pdf->SetXY($this->posxref -1, $curY);
					$pdf->MultiCell($this->colwidthref, 4, $barcode, 0, 'L');
					

					// Quantity
					$qty = pdf_getlineqty($object, $i, $outputlangs, $hidedetails);
					$pdf->SetXY($this->posxqty, $curY);
					$pdf->MultiCell($this->colwidthqty - 0.8, 4, $qty, 0, 'R'); // Enough for 6 chars
					if ($noQty != $outputlangs->transnoentities("shippingPresta") && $noQty != $outputlangs->transnoentities("discountPresta")) {
						$totalQty += $qty;
					...
					if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT) && empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_COLUMN)) {
						...
						$pdf->SetXY($this->posxtvatx +1 , $curY);
						$pdf->MultiCell($this->colwidthtvatx -0.8, 3, $vat_rate, 0, 'L');
					}

					// Unit price before discount
					$up_excl_tax = pdf_getlineupexcltax($object, $i, $outputlangs, $hidedetails);
					$pdf->SetXY($this->posxpuht, $curY);
					$pdf->MultiCell($this->colwidthpuht - 0.8, 3, $up_excl_tax, 0, 'R', 0);					

					// Discount on line
					if ($object->lines[$i]->remise_percent) {
						$pdf->SetXY($this->posxrem - 2, $curY);
						$remise_percent = pdf_getlineremisepercent($object, $i, $outputlangs, $hidedetails);
						$pdf->MultiCell($this->colwidthrem + 2, 3, $remise_percent, 0, 'R');
					}
					
					// Total HT line
					$total_excl_tax = pdf_getlinetotalexcltax($object, $i, $outputlangs, $hidedetails);
					$pdf->SetXY($this->posxtotalht, $curY);
					$pdf->MultiCell($this->colwidthtotalht, 3, $total_excl_tax, 0, 'R', 0);

					// unit price with discount line
					if ($object->lines[$i]->remise_percent) {
						$pdf->SetXY($this->posxrht - 2, $curY);
						$unit_price_remise_ht = $object->lines[$i]->total_ht / $object->lines[$i]->qty;
						$unit_price_remise_ht =  price($unit_price_remise_ht , 0, $outputlangs);
						$pdf->MultiCell($this->colwidthprht + 2, 3,$unit_price_remise_ht, 0, 'R');
					}
				...
				// Display total area
				$posy = $this->_tableau_tot($pdf, $object, $deja_regle, $bottomlasttab, $outputlangs, $outputlangsbis, $totalQty);
	 ...
	 */
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs, $outputlangsbis,  $totalQty)
	{
		// phpcs:enable
		...
		$pdf->SetTextColor(0, 0, 0);		
		$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("TotalQty"), $useborder, 'L', 1);
		$pdf->SetXY($col2x, $tab2_top + 0);
		$pdf->MultiCell($largcol2, $tab2_hl,  $totalQty, $useborder, 'R', 1);
		...
		$pdf->MultiCell($largcol2, $tab2_hl, preg_replace('~\,00+$~','',price($sign * ($total_ht + (!empty($object->remise) ? $object->remise : 0)), 0, $outputlangs)), 0, 'R', 1);
			...
			if (!empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_IFNULL) && $tvaisnull) {
				...
				foreach ($this->tva_array as $tvakey => $tvaval) {
					if ($tvakey != 0) {    // On affiche pas taux 0
						...
						$pricevattotal = price(price2num($tvaval['amount'], 'MT'), 0, $outputlangs);
						$pricevattotal = preg_replace("/[^0-9,]/","",$pricevattotal);
						$pricevattotal = preg_replace("/[,]/",".",$pricevattotal);
						$pricevattotal = round($pricevattotal,0);
						$pricevattotal = number_format($pricevattotal, 0, ',', ' ');

						$pdf->MultiCell($largcol2, $tab2_hl, $pricevattotal, 0, 'R', 1);
					}
				}
				...
				if (price2num($object->revenuestamp) != 0) {
					...
					$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
					$pdf->MultiCell($largcol2, $tab2_hl, preg_replace('~\,00+$~','',price($sign * $object->revenuestamp)), $useborder, 'R', 1);
				}
				...
				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($largcol2, $tab2_hl, preg_replace('~\,00+$~','',price($sign * $total_ttc, 0, $outputlangs)), $useborder, 'R', 1);
				...
				if ($object->displayRetainedWarranty()) {
					...
					$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
					$pdf->MultiCell($largcol2, $tab2_hl, preg_replace('~\,00+$~','',price($billedWithRetainedWarranty)), $useborder, 'R', 1);
					...
					$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
					$pdf->MultiCell($largcol2, $tab2_hl, preg_replace('~\,00+$~','',price($retainedWarranty)), $useborder, 'R', 1);
				}
			}
		}
		...
		if (($deja_regle > 0 || $creditnoteamount > 0 || $depositsamount > 0) && empty($conf->global->INVOICE_NO_PAYMENT_DETAILS)) {
			...
			$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($largcol2, $tab2_hl, preg_replace('~\,00+$~','',price($deja_regle + $depositsamount, 0, $outputlangs)), 0, 'R', 0);
			...
			if ($creditnoteamount) {
				...
				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($largcol2, $tab2_hl, preg_replace('~\,00+$~','',price($creditnoteamount, 0, $outputlangs)), 0, 'R', 0);
			}
			...
			if ($object->close_code == Facture::CLOSECODE_DISCOUNTVAT) {
				...
				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($largcol2, $tab2_hl, preg_replace('~\,00+$~','',price($object->total_ttc - $deja_regle - $creditnoteamount - $depositsamount, 0, $outputlangs)), $useborder, 'R', 1);
			...
			$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($largcol2, $tab2_hl, preg_replace('~\,00+$~','',price($resteapayer, 0, $outputlangs)), $useborder, 'R', 1);
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '')
	{
		...
		if (empty($hidetop)) {	
			$pdf->line($this->marge_gauche, $tab_top + 8, $this->page_largeur - $this->marge_droite, $tab_top + 8); // line takes a position y in 2nd parameter and 4th parameter		
			$pdf->SetXY($this->posxref - 1, $tab_top + 1);
			$pdf->MultiCell($this->colwidthref, 2, $outputlangs->transnoentities("Référence"), '', 'L', '', '', '', '', '', '', '', '', '', 'M');
		}
		$pdf->line($this->posxdesc, $tab_top, $this->posxdesc - 1, $tab_top + $tab_height); // line takes a position y in 2nd parameter and 4th parameter
		if (empty($hidetop)) {
			$pdf->SetXY($this->posxdesc + 1, $tab_top + 1);
			$pdf->MultiCell(108, 2, $outputlangs->transnoentities("Designation"), '', 'L');
		}
		$pdf->line($this->posxqty, $tab_top, $this->posxqty - 1, $tab_top + $tab_height); // line takes a position y in 2nd parameter and 4th parameter
		if (empty($hidetop)) {
			$pdf->SetXY($this->posxqty + 1, $tab_top + 1);
			$pdf->MultiCell($this->colwidthqty, 2, $outputlangs->transnoentities("QTY"), '', 'L');
		}
		$pdf->line($this->posxpuht, $tab_top, $this->posxpuht - 1, $tab_top + $tab_height); // line takes a position y in 2nd parameter and 4th parameter
		if (empty($hidetop)) {
			$pdf->SetXY($this->posxpuht + 1, $tab_top + 1);
			$pdf->MultiCell($this->colwidthpuht, 2, $outputlangs->transnoentities("PUHT"), '', 'L');
		}
		$pdf->line($this->posxrem, $tab_top, $this->posxrem - 1, $tab_top + $tab_height); // line takes a position y in 2nd parameter and 4th parameter
		if (empty($hidetop)) {
			$pdf->SetXY($this->posxrem + 1, $tab_top + 1);
			$pdf->MultiCell($this->colwidthrem, 2, $outputlangs->transnoentities("REM"), '', 'L');
		}
		$pdf->line($this->posxrht, $tab_top, $this->posxrht - 1, $tab_top + $tab_height); // line takes a position y in 2nd parameter and 4th parameter
		if (empty($hidetop)) {	
			$pdf->SetXY($this->posxrht + 1, $tab_top + 1);			
			$pdf->MultiCell($this->colwidthprht, 2, $outputlangs->transnoentities("RHT"), '', 'L');
		}
		$pdf->line($this->posxtotalht, $tab_top, $this->posxtotalht - 1, $tab_top + $tab_height); // line takes a position y in 2nd parameter and 4th parameter
		if (empty($hidetop)) {
			$pdf->SetXY($this->posxtotalht + 1, $tab_top + 1);
			$pdf->MultiCell($this->colwidthtotalht, 2, $outputlangs->transnoentities("TOTALHT"), '', 'L');
		}
		$pdf->line($this->posxtvatx, $tab_top, $this->posxtvatx - 1, $tab_top + $tab_height); // line takes a position y in 2nd parameter and 4th parameter
		if (empty($hidetop)) {
			$pdf->SetXY($this->posxtvatx + 1, $tab_top + 1);
			$pdf->MultiCell($this->colwidthtvatx, 2, $outputlangs->transnoentities("TGC"), '', 'L');
		}
	}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null)
	{
		...
		if (empty($conf->global->PDF_DISABLE_MYCOMPANY_LOGO)) {
			if ($this->emetteur->logo) {
				...
				$extrafieldssoc = new ExtraFields($this->db);
				$extralabelssoc = $extrafieldssoc->fetch_name_optionals_label('societe', true);

				if (count($extralabelssoc) > 0) {

					$societe = new Societe($this->db);

					$societe->fetch($object->socid);
					$societe->fetch_optionals($object->socid);
					$interfacepdfsoc = $societe->array_options['options_interfacepdf'];
				}

				$interfacepdfdoc = $object->array_options['options_interfacepdf'];
				//$pdf->MultiCell(0, 3,$logodir);
				if ($interfacepdfsoc == 'bookin') {
					if ($interfacepdfdoc == 'caledolivres') {
						$logo = $logodir . '/logos/thumbs/' . $this->emetteur->logo_small;
					} else {
						$logo = $logodir . '/logos/thumbs/bookin-dis.png';
					}
				} else {
					if ($interfacepdfdoc == 'bookin') {
						$logo = $logodir . '/logos/thumbs/bookin-dis.png';
					} else {
						$logo = $logodir . '/logos/thumbs/' . $this->emetteur->logo_small;
					}
				...
				if (is_readable($logo)) {
					...
					if ($interfacepdfsoc == 'bookin') {
						if ($interfacepdfdoc == 'caledolivres') {
							...
							$pdf->Image($logo, $this->marge_gauche, $posy, 0, $conf->global->HEIGHT_LOGO_BOOKIN);							
						}
					} else {
						if ($interfacepdfdoc == 'bookin') {
							$pdf->Image($logo, $this->marge_gauche, $posy, 0, $conf->global->HEIGHT_LOGO_BOOKIN);
		...
		if ($notetoshow) {
			$posy += 4;
			$pdf->SetXY($posx, $posy+0.8);
			$pdf->SetTextColor(0, 0, 0);
			$pdf->MultiCell($w, 3, $outputlangs->transnoentities("DocRemark") . " : " . $outputlangs->convToOutputCharset($notetoshow), '', 'R');
		}
		...
		if ($showaddress) {
			...
			if (empty($conf->global->MAIN_PDF_NO_SENDER_FRAME)) {
				...
				$pdf->MultiCell($widthrecbox - 2, 5, $outputlangs->transnoentities("BillFrom"), 0, $ltrdirection);
				$pdf->SetXY($posx, $posy);
				$pdf->SetFillColor(255, 255, 255);				
			...
			if ($interfacepdfsoc == 'bookin') {
				if ($interfacepdfdoc == 'caledolivres') {
					$sender_name = 'Calédo Livres';
				} else {
					$sender_name = 'Book’in distribution';
				}
			} else {
				if ($interfacepdfdoc == 'bookin') {
					$sender_name = 'Book’in distribution';
			...
			$pdf->SetXY($posx + 2, $posy + 3);
			$pdf->SetFont('', 'B', $default_font_size);
			$pdf->MultiCell($widthrecbox - 2, 4, $sender_name, 0, $ltrdirection);
			$posy = $pdf->getY();
			...
			//$pdf->MultiCell(0, 3,$logodir);
			/*$outputlangs->transnoentities("headerCLD1");
			$outputlangs->transnoentities("headerCLD2");
			$outputlangs->transnoentities("headerCLD3");
			$outputlangs->transnoentities("headerCLD4");
			$outputlangs->transnoentities("headerCLD5");
			$outputlangs->transnoentities("headerCLD6");
			$outputlangs->transnoentities("headerCLD7");
			$outputlangs->transnoentities("headerCLD8");
			$outputlangs->transnoentities("headerCLD9");
			$outputlangs->transnoentities("headerBKN1");
			$outputlangs->transnoentities("headerBKN2");
			$outputlangs->transnoentities("headerBKN3");
			$outputlangs->transnoentities("headerBKN4");
			$outputlangs->transnoentities("headerBKN5");
			$outputlangs->transnoentities("headerBKN6");*/
			$carac_emetteur_cale = '';
			for ($i = 1; $i <= 9; $i++) {
				$valeur = $outputlangs->transnoentities("headerCLD$i");
				if (!empty($valeur)) {
					$carac_emetteur_cale .= $valeur . "\n";

					// Ajouter un saut de ligne supplémentaire après la 3e et la 5e valeur
					if ($i == 3 || $i == 5) {
						$carac_emetteur_cale .= "\n";
					}
				}
			}
			// Supprimer le dernier saut de ligne si nécessaire
			$carac_emetteur_cale = rtrim($carac_emetteur_cale, "\n");
			
			$carac_emetteur_book = '';

			for ($i = 1; $i <= 6; $i++) {
				$valeur = $outputlangs->transnoentities("headerBKN$i");
				if (!empty($valeur)) {
					$carac_emetteur_book .= $valeur . "\n";
			...
			// Supprimer le dernier saut de ligne si nécessaire
			$carac_emetteur_book = rtrim($carac_emetteur_book, "\n");

			//$carac_emetteur_book = '21, rue Jean-Jaurès' . "\n" . 'BP 32044, 98897 Nouméa cedex' . "\n" . 'Nouvelle-Calédonie' . "\n\n" . 'Tél.fixe (687) 28 38 03' . "\n" . 'Tél.Mob (687) 75 20 34' . "\n" . 'Courriel : contact@bookin.nc';

			if ($interfacepdfsoc == 'bookin') {
				if ($interfacepdfdoc == 'caledolivres') {
					$carac_emetteur = $carac_emetteur_cale;
				} else {
					$carac_emetteur = $carac_emetteur_book;
				}
			} else {
				if ($interfacepdfdoc == 'bookin') {
					$carac_emetteur = $carac_emetteur_book;
	...
	protected function _pagefoot(&$pdf, $object, $outputlangs, $hidefreetext = 0)
	{
		...
		$footer_bkn_1 = $outputlangs->transnoentities("footerBKN1");
		$footer_bkn_2 = $outputlangs->transnoentities("footerBKN2");
		$footer_bkn_3 = $outputlangs->transnoentities("footerBKN3");
		$footer_cld_1 = $outputlangs->transnoentities("footerCLD1");
		$footer_cld_2 = $outputlangs->transnoentities("footerCLD2");

		$nb_page = $pdf->PageNo() . '/' . $pdf->getAliasNbPages();

		if ($interfacepdfsoc == 'bookin') {
			if ($interfacepdfdoc == 'caledolivres') {
				$pdf->SetXY(10, 282);
				$pdf->SetFont('', '', 6);
				$pdf->MultiCell(0, 4, $footer_cld_1, 0, 'C');

				$pdf->SetXY(10, 289);
				$pdf->SetFont('', '', 6);
				$pdf->MultiCell(0, 4, $footer_cld_2, 0, 'C');

				$pdf->SetXY(0, 275);
				$pdf->SetFont('', '', 10);
				$pdf->MultiCell(0, 0, $nb_page, 0, 'R');
			} else {
				$pdf->SetXY(10, 280);
				$pdf->SetFont('', '', 6);
				$pdf->MultiCell(0, 4, $footer_bkn_1, 0, 'C');

				$pdf->SetXY(10, 285.5);
				$pdf->SetFont('', 'B', 6);
				$pdf->MultiCell(0, 4, $footer_bkn_2, 0, 'C');

				$pdf->SetXY(10, 289);
				$pdf->SetFont('', '', 6);
				$pdf->MultiCell(0, 4, $footer_bkn_3, 0, 'C');
	 ...
	 * 	@return	string
	 */
	function pdf_writelinedesc_cale(&$pdf, $object, $i, $outputlangs, $w, $h, $posx, $posy, $hideref = 0, $hidedesc = 0, $issupplierline = 0)
	{
		...
		if (is_object($hookmanager)) {   // Old code is commented on preceding line. Reproduct this test in the pdf_xxx function if you don't want your hook to run
			...
			$action = '';
			$reshook = $hookmanager->executeHooks('pdf_writelinedesc_cale', $parameters, $object, $action); // Note that $action and $object may have been modified by some hooks
		...
		if (empty($reshook)) {
			...
			$static_product = new Product($db);
			$static_product->fetch($object->lines[$i]->fk_product);
			$static_product->fetch_optionals($object->lines[$i]->fk_product);
			$options_auteur = $static_product->array_options['options_auteur'];
			$options_editeur = $static_product->array_options['options_editeur'];
			if ($labelproductservice == 'shipping') {
				$labelproductservice = $outputlangs->transnoentities("shippingPresta");								
			}
			if ($labelproductservice == 'discount') {
				$labelproductservice = $outputlangs->transnoentities("discountPresta");
			}
			if ($options_auteur !='' && $options_editeur !='') {
				$labelproductservice = $labelproductservice.'<br><span style="font-size:6pt;">'.$options_auteur.' - '.$options_editeur.'</span>';
			} elseif ($options_auteur !='' && $options_editeur =='') {
				$labelproductservice = $labelproductservice.'<br><span style="font-size:6pt;">'.$options_auteur.'</span>';
			} elseif ($options_auteur =='' && $options_editeur !='') {
				$labelproductservice = $labelproductservice.'<br><span style="font-size:6pt;">'.$options_editeur.'</span>';
			}
	...
	function pdf_getlinebarcode($object, $i, $outputlangs, $hidedetails = 0)
	{
		...
		$static_product = new Product($db);
		$static_product->fetch($object->lines[$i]->fk_product);
		return dol_htmlentitiesbr($static_product->barcode);
	}
}